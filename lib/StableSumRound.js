function stableSumRound(precision, values) {
  ...
  const valuesToAdjust = values
    .map((v, i) => {
      return {
        original: v,
        originalIndex: i,
        rounded: round(precision, v)
      }
    })
    .map(v => {
      return {
        ...v,
        diff: v.original - v.rounded
      }
    });
  const roundedSum = valuesToAdjust.reduce((a, b) => a + b.rounded, 0);
  const adjustmentCount = Math.abs(Math.round((roundedSum - sum) / decimal));
  const gt = roundedSum > sum;
  const adjustmentAmount = gt ? decimal * -1 : decimal;
  ...
  const adjustedValues = sorted.map(function(v, i) {
    return {
      ...
      rounded: i < adjustmentCount ? v.rounded + adjustmentAmount : v.rounded
    }
  });
  ...
  const finalValues = adjustedValues
    ...
    .map(function(v) {
      return round(precision, v.rounded)
    });
  return finalValues;
}
