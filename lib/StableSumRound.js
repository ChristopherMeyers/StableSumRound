function stableSumRound(precision, values) {
  ...
  const valuesToAdjust = values.map((v, i) => { return { original: v, originalIndex: i, rounded: round(precision, v) } })
  ...
  const adjustedValues = sorted.map(function(v, i) {
    return{ originalIndex: v.originalIndex, rounded: i < adjustmentCount ? v.rounded + adjustmentAmount : v.rounded }
  });

  const finalValues = adjustedValues
    .sort(function(a, b) { return a.originalIndex > b.originalIndex ? 1 : -1; })
